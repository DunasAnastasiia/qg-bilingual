...
class Config:
    ...
    def _resolve_paths(self):
        is_docker = (os.path.exists("/.dockerenv") and os.name != "nt") or os.getenv(
            "DOCKER_ENV"
        ) == "true"
        ...
        self.checkpoint_dir = resolve_env_path(
            "CHECKPOINT_DIR", "/app/checkpoints", "checkpoints"
        )
        self.logs_dir = resolve_env_path("LOGS_DIR", "/app/logs", "logs")
        self.experiments_dir = resolve_env_path(
            "EXPERIMENTS_DIR", "/app/experiments", "experiments"
        )
