    ...
    def preprocess_function(self, examples: Dict) -> Dict:
        ...
        for i in range(len(examples["context"])):
            ...
            if self.mode == "answer_aware":
                inputs.append(
                    f"{q_prefix}: {c_prefix}: {context} {a_prefix}: {examples['answer'][i]}"
                )
            else:
                inputs.append(f"{q_prefix}: {c_prefix}: {context}")
        model_inputs = self.tokenizer(
            inputs, max_length=self.max_source_length, truncation=True
        )
        labels = self.tokenizer(
            examples["question"], max_length=self.max_target_length, truncation=True
        )
