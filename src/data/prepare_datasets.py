    ...
    try:
        ...
        if demo_mode:
            ...
            dataset["train"] = (
                dataset["train"]
                .shuffle(seed=42)
                .select(range(min(demo_size, len(dataset["train"]))))
            )
            dataset["validation"] = (
                dataset["validation"]
                .shuffle(seed=42)
                .select(range(min(demo_size // 5, len(dataset["validation"]))))
            )
...
def prepare_ukrainian_dataset(output_path=None, demo_mode=False, demo_size=1000):
    ...
    try:
        dataset = load_dataset(
            "nogyxo/question-answering-ukrainian", verification_mode="no_checks"
        )
        ...
        if demo_mode:
            ...
            dataset["test"] = (
                dataset["test"]
                .shuffle(seed=42)
                .select(range(min(demo_size // 5, len(dataset["test"]))))
            )
            ...
            for split in ["train", "test"]:
                for example in tqdm(dataset[split], desc=f"Saving Ukrainian {split}"):
                    record = {
                        ...
                        "question": example["question"],
                        "answer": (
                            example["answer_text"] if example["answer_text"] else ""
                        ),
                        "all_answers": (
                            [example["answer_text"]] if example["answer_text"] else []
                        ),
