...
class DatasetLoader:
    ...
    def load_squad_v2(
        self, filter_unanswerable: bool = False, deduplicate_by_context: bool = False
    ) -> DatasetDict:
        dataset = load_dataset("squad_v2")
        dataset = dataset.map(
            self._process_squad_example, remove_columns=["id", "title"], num_proc=4
        )
            ...
            dataset = DatasetDict(
                {
                    split: dataset[split].filter(
                        lambda x: not x["unanswerable"], num_proc=4
                    )
    ...
    def load_ukrainian_dataset(
        self,
        file_path: Path,
        filter_unanswerable: bool = False,
        deduplicate_by_context: bool = False,
    ) -> Dataset:
        examples = []
    ...
    def _process_ukrainian_example(self, data: Dict) -> Optional[Dict]:
        ...
        is_impossible = data.get("is_impossible", False) or data.get(
            "unanswerable", False
        )
    ...
    def filter_by_length(self, dataset: Dataset, config: dict) -> Dataset:
        def length_filter(example):
            ...
            return (
                config["min_context_len"] <= ctx_len <= config["max_context_len"]
                and config["min_question_len"] <= q_len <= config["max_question_len"]
                and (
                    example["unanswerable"]
                    or config["min_answer_len"] <= ans_len <= config["max_answer_len"]
                )
            )
    ...
    def stratified_split(
        self, dataset: Dataset, train_ratio: float, val_ratio: float, seed: int
    ) -> DatasetDict:
        total_size = len(dataset)

        if total_size < 3:
            print(
                f"Warning: Dataset too small ({total_size} samples). Using all data for train, validation, and test."
            )
            return DatasetDict(
        ...
        if min_train_size < 1 or min_val_size < 1 or min_test_size < 1:
            print(
                f"Warning: Cannot properly split {total_size} samples. Duplicating data across splits."
            )
            train_size = max(1, int(total_size * 0.8))
            dataset = dataset.train_test_split(
                test_size=total_size - train_size, seed=seed
            )
            return DatasetDict(
                {
                    "train": dataset["train"],
        ...
        val_test = dataset["test"].train_test_split(
            test_size=1 - val_test_ratio, seed=seed
        )
        result = DatasetDict(
            {
                "train": dataset["train"],
    ...
    def _apply_limits(self, dataset: DatasetDict) -> DatasetDict:
        ...
        for split_name, split_data in dataset.items():
            ...
            if limit < current_size:
                print(
                    f"Limiting {split_name} split from {current_size} to {limit} examples"
                )
