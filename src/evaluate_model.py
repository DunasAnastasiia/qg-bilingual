...
def generate_questions(model, tokenizer, dataset, config, device='cuda', max_samples=None, qa_model: 'QAModel' = None):
    ...
    for item in tqdm(eval_items, desc="Generating"):
        ...
        if candidates:
            ...
            if lang == 'en':
                wh_regex = re.compile(r"^(who|what|when|where|why|how|which|whose|whom|is|are|do|does|did|can|could|will|would|should|may|might|must|has|have|had)\b", re.I)
            elif lang == 'ua':
                wh_regex = re.compile(r"^(хто|що|де|коли|куди|звідки|як|чому|навіщо|який|чий|скільки|чи|яка|яке|які|чий|чия|чиє|чиї)\b", re.I)
            else:
                wh_regex = None
            ...
            if qa_model is not None and gold_ans:
                qa_results = qa_model.answer_question_batch(candidates, [context] * len(candidates))
            
            for idx, cand in enumerate(candidates):
                try:
                    if qa_results and idx < len(qa_results):
                        qa_out = qa_results[idx]
                        f1s = [qa_model.compute_em_f1(qa_out.get('answer', ''), ga)[1] for ga in gold_ans]
                    ...
                    overlap = sum(1 for w in cand_words if w in ctx_words) / len(cand_words) if cand_words else 1.0
                    copy_penalty = -1.0 if overlap > 0.85 else (-0.4 if overlap > 0.7 else 0.0)

                    ans_in_q_penalty = 0.0
                    if qa_results and idx < len(qa_results):
                        qa_ans = qa_results[idx].get('answer', '')
                        if qa_ans and len(qa_ans) > 3 and qa_ans.lower() in cand.lower():
                            ans_in_q_penalty = -1.0
                    pass_bonus_val = 2.0 if mode == 'answer_aware' else 10.0
                    pass_bonus = pass_bonus_val if cand_f1 >= 0.8 and qa_conf >= 0.35 else 0.0
                    score = (cand_f1 * 2.0) + pass_bonus + wh_bonus + copy_penalty + ans_in_q_penalty + (qa_conf * 0.5)
                except Exception:
                    score = -1.0
...
def compute_all_metrics(generated_questions, gold_questions, contexts, gold_answers,
                        qa_model, metrics_calc, config):
    print("\n📊 Computing metrics...")
...
def compute_pass_rate(metrics, f1_threshold=0.8):
    if 'qa_pass_rate' in metrics and 'qa_pass_count' in metrics and 'qa_total' in metrics:
