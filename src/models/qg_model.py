...
class QGModel:
    ...
    def _apply_lora(self):
        ...
        lora_config = LoraConfig(
            ...
            lora_alpha=self.config["lora"].get(
                "lora_alpha", self.config["lora"].get("alpha", 32)
            ),
            lora_dropout=self.config["lora"].get(
                "lora_dropout", self.config["lora"].get("dropout", 0.1)
            ),
            target_modules=self.config["lora"].get(
    ...
    def get_training_args(self, output_dir: str) -> Seq2SeqTrainingArguments:
        ...
        if (
            self.config["training"].get("fp16", False)
            or self.config["training"].get("bf16", False)
        ) and self.device == "cpu":
            print(
                "⚠ WARNING: fp16/bf16 disabled (CPU doesn't support mixed precision training)"
            )

        dataloader_num_workers = self.config["training"].get(
            "dataloader_num_workers", 0
        )
        if os.name == "nt" and dataloader_num_workers > 0:
            if (
                self.config["training"].get("gradient_checkpointing", False)
                or self.compiled
            ):
                reason = (
                    "gradient_checkpointing is enabled"
                    if self.config["training"].get("gradient_checkpointing", False)
                    else "torch.compile is enabled"
                )
                print(
                    f"⚠ WARNING: Setting dataloader_num_workers=0 on Windows because {reason} (avoids pickling errors)"
                )
                dataloader_num_workers = 0
        ...
        kwargs = {
            ...
            "label_smoothing_factor": self.config["training"].get(
                "label_smoothing", 0.0
            ),
            ...
            "generation_num_beams": self.config["training"].get(
                "generation_num_beams", 1
            ),  # Default to greedy for speed
            "seed": self.config["seed"],
            "gradient_checkpointing": self.config["training"].get(
                "gradient_checkpointing", False
            ),
            "dataloader_num_workers": dataloader_num_workers,
            "dataloader_pin_memory": self.config["training"].get(
