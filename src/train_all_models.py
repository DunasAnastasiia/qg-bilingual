...
MODELS_TO_TRAIN: List[Dict[str, str]] = [
    {
        "name": "t5_base_en_aware",
        "config": "train_t5_base.yaml",
        "description": "T5-base English Answer-Aware",
    },
    {
        "name": "t5_base_en_agnostic",
        "config": "train_t5_base.yaml",
        "description": "T5-base English Answer-Agnostic",
        "override_mode": "answer_agnostic",
    },
    {
        "name": "bart_base_en_aware",
        "config": "train_bart_base_aware_en.yaml",
        "description": "BART-base English Answer-Aware",
    },
    {
        "name": "bart_base_en_agnostic",
        "config": "train_bart_base_agnostic_en.yaml",
        "description": "BART-base English Answer-Agnostic",
    },
    {
        "name": "mt5_base_ua_aware",
        "config": "train_mt5_base_aware_ua.yaml",
        "description": "mT5-base Ukrainian Answer-Aware",
    },
    {
        "name": "mt5_base_ua_agnostic",
        "config": "train_mt5_base_aware_ua.yaml",
        "description": "mT5-base Ukrainian Answer-Agnostic",
        "override_mode": "answer_agnostic",
    },
]
...
def train_model(model_config: Dict[str, str]) -> bool:
    ...
    try:
        ...
        subprocess.run(
            cmd, cwd=ROOT_DIR, check=True, capture_output=False, text=True, env=env
        )
...
def main():
    ...
    for i, model_config in enumerate(MODELS_TO_TRAIN, 1):
        logger.info(
            f"[{i}/{len(MODELS_TO_TRAIN)}] Processing {model_config['name']}..."
        )
    ...
    logger.info(
        f"Training Summary - Total: {len(MODELS_TO_TRAIN)}, Success: {trained_count}, Failed: {failed_count}"
    )
