...
class MetricsVisualizer:
    ...
    def plot_training_curves(
        ...
        val_rouge_scores: List[float],
        save_name: str = "training_curves.png",
    ):
        if not train_losses or not val_losses or not val_rouge_scores:
            print(
                f"Warning: Skipping plot - empty data (train: {len(train_losses)}, val: {len(val_losses)}, rouge: {len(val_rouge_scores)})"
            )
            return
        ...
        ax2.plot(
            epochs, val_rouge_scores, label="Val ROUGE-L", marker="o", color="green"
        )
